...
function get_cat_list()
{
	...
	if ($cats === NULL) {
		...
		if ($data === false) {
			$sql = "select a.catid, a.catname, a.catorder as catorder , b.description as description, b.catid as childid, b.catname as childname, b.catorder as chiorder from {$table}category as a left join {$table}category as b on b.parentid = a.catid where a.parentid = '0' order by catorder,a.catid,chiorder asc";
			$res = $db->getAll($sql);
			...
			foreach ($res as $row) {
				...
				$cats[$row['catid']]['description'] = $row['description'];
				...
				if(!empty($row['childid'])) {
					...
					$cats[$row['catid']]['children'][$row['childid']]['description'] = $row['description'];
...
{
	$here = '<li><a href="'.PHPMPS_PATH.'">首页</a><span class="divider">/</span></li>';
	foreach($here_arr as $val) {
		if(!empty($val['url']) && !empty($val['name'])) {
			$here .= '<li><a href="'.$val['url'].'">' . $val['name'] . '</a><span class="divider">/</span></li>';
		} elseif (empty($val['url']) && !empty($val['name'])) {
			$here .= '<li class="active">'. $val['name'].'</li>';
		}
	}
	return $here;
}
